...
`/alpha-report` 只在浏览器本地生成可复制文本，不提交数据、不保存浏览器指纹、不写入数据库。玩家把复制出来的文本发给维护者即可。单人局会从浏览器本地自动带出“反馈编号”；玩家中途或终局也可以在对局面板里复制同一个编号。
...
6. 如果是单人局 AI 发言/流程问题，优先让玩家提供反馈编号；如果玩家没有提供编号，用 owner token 访问 `GET /api/games/samples?token=...` 按时间找最近单人局，再用 `GET /api/games/:gameId/sample?token=...` 导出公开样本。
7. 对 P0/P1 建一个小修复任务；每次只修一个明确问题并跑 smoke。
...
- `/alpha-report` 不是在线表单，不会自动汇总文本反馈；单人局 AI 样本需要维护者用 owner-token 接口按反馈编号或最近样本索引导出。
