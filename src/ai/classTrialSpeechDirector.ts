...
export function buildClassTrialOpeningDirectorGuide(
  ...
  return [
    ...
    "不要把系统发言顺序说成狼队能故意安排的谜题；只能审已经说出口的接话方式、压力转向和公开身份说法。",
...
export function buildClassTrialRepeatedFocusGuide(
  ...
  return `动态导演提示：前置位已经多次围绕「${motif.label}」打转；本轮不要只复读这类话。如果反复被抓的是同一个人的没给倾向、没给票口或缺口，不能把这句作为主轴，必须转成这个角色会抓住的证词、声音、伪装、下注、合格线、关系链或反应差。${motif.shift} ${rolePressure} 换一个镜头：具体事实、身份说法、反应差、死亡形态或角色会注意到的选择；如果没有新事实，宁可短句保留。`;
}
