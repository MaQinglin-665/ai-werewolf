...
function buildRoundFrameNote(view: AgentView): string {
  if (view.day <= 1) {
    return "轮次意识：首日先分清身份声明质量、站边理由和发言顺序；低证据听感只能施压，不要直接当铁狼定案。";
  }
...
function buildIdentityPitNote(view: AgentView): string {
  ...
  if (counterclaims.length > 0) {
    const claimText = counterclaims
      ...
      .join("；");
    return `身份坑审计：${claimText}。先比身份线内部逻辑，再看外置位谁在帮忙煽动、分票或回避站边。`;
  }
  ...
  if (powerClaims.length > 0) {
    const claimText = powerClaims
      ...
      .join("、");
    return `身份坑审计：公开神职声明有${claimText}；未对跳神职先保护，但要继续校验发言、站边和票型是否一致。`;
  }

  return "身份坑审计：当前没有公开神职对跳，闭眼位先用发言过程和票型压缩狼坑，不要逼神职过早拍身份。";
}
...
function buildSheriffAuditNote(view: AgentView): string | undefined {
  ...
  if (sheriffVote?.revealed && sheriffVote.tally.length > 0) {
    return `警长审计：警徽票型为${formatTally(sheriffVote.tally)}；高阶局不能只按团队大小定真假，要看投票理由、站边一致性和是否破坏真预言家警徽流。`;
  }

  const activeCandidates = sheriff.candidates.filter((candidate) => !sheriff.withdrawnSeatIds.includes(candidate.seatId));
  if (activeCandidates.length > 0) {
    return `警长审计：当前警上候选为${formatSeatList(activeCandidates)}；发言要检查报验人、警徽流、心路历程和警下投票预期是否连成一条线。`;
  }

  if (sheriff.badgeHolder) {
    return `警长审计：当前警徽在${seatText(sheriff.badgeHolder)}；后续归票要看警长是否把发言、身份线和票型组织到同一方向。`;
  }

  return "警长审计：本局有警长流程但还没有形成公开警徽票型，先保留警上发言质量和警下投票任务。";
}
...
function buildVoteAuditNote(view: AgentView): string {
  ...
  if (latestVote?.tally.length) {
    return `票型复盘：上一轮公开票型为${formatTally(latestVote.tally)}；重点追问谁起票、谁补票、谁最后跟票，以及理由是否和发言闭环。`;
  }

  if (view.phase === "DAY_VOTE") {
    return "票型复盘：当前还没有可复盘的公开放逐票型，投票理由必须来自查验、身份线、站边变化或已发言内容。";
  }

  return "票型复盘：还没有公开放逐票型时，先记录每个人的站边和出人意愿，等投票后校验是否前后一致。";
}
...
function buildSeerClaimAuditNote(view: AgentView): string | undefined {
  ...
  if (seerClaims.length > 1) {
    const claimText = seerClaims
      ...
      .join("；");
    return `预言家线审计：${claimText}。先比较验人力度、起跳时机、金水/查杀回应和双方票型，再决定站边。`;
  }

  if (seerClaims.length === 1) {
    const claim = seerClaims[0]!;
    const checkText = claim.checks.length ? `，${claim.checks.map(formatCheck).join("、")}` : "";
    return `预言家线审计：${seatText(claim.claimant)}单边声称预言家${checkText}；单边身份也要落到验人、发言质量和票型收益上。`;
  }
...
function buildDeathLegacyNote(view: AgentView): string | undefined {
  ...
  if (legacy) {
    return `遗言和倒牌审计：${legacy.summary}；夜死预言家声明只按公开遗留处理，不等于系统确认真预言家。`;
  }
...
function buildRoleTaskNote(view: AgentView): string | undefined {
  if (view.myRole === "SEER") {
    return view.privateKnowledge.sheriff
      ? "本身份任务：预言家公开发言要同时交验人、警徽流和心路历程；警徽流优先验能打开狼坑或校验站边的人。"
      : "本身份任务：无警徽时预言家更要把验人、站边和今天票口说清，避免只报身份不组织桌面。";
  }

  if (view.myRole === "VILLAGER") {
    return "本身份任务：平民没有夜间信息，重点是压缩身份坑、保护未对跳神职、用公开发言和票型找狼。";
  }

  if (isWolfRole(view.myRole, view.rules.wolfRoles)) {
    return "本身份任务：狼人先选悍跳、冲票、倒钩或卖队友的战术，再用公开发言、身份线和票型包装理由。";
  }

  if (POWER_ROLES.includes(view.myRole)) {
    return "本身份任务：神职拍身份要服务轮次和票型；不需要拍时先用公开逻辑发言，避免无收益交身份给夜刀。";
  }
