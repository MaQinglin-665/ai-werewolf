import { leaveRoomSession } from "@/server/roomService";
import { z } from "zod";
...
export async function POST(request: Request, { params }: { params: Promise<{ roomId: string }> }) {
  ...
  if (!parsed.success) {
    return Response.json({ error: "离开房间参数不合法。", details: parsed.error.flatten() }, { status: 400 });
  }

  try {
    return Response.json(
      await leaveRoomSession(roomId, { playerId: parsed.data.playerId, playerToken: parsed.data.playerToken }, {
